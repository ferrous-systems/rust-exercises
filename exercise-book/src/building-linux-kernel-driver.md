...
Then we're going to resize it because it's too small (both the AArch64 one and
the x86-64 one).
...
We're not going to boot the VM.
...
favourite telnet client) on `localhost:5555` You should end up at a login
...
filesystem (the bottom one) and pick `[ Resize ]`, then `[ Write ]`, and `[ Quit
]`. You'll need to type `yes` in response to `[ Write ]`.
...
the root filesystem to fill the partition.
