# Changing to Nonsecure State

We got Secure State running in the previous chapter. But we also want to run a
different application in Nonsecure State at the same time.
...
By default the SAU will say no to anything the Nonsecure State asks for (which is
...
Secure State, and give all of SRAM3 to Nonsecure State. As SRAM3 is 832 KiB in
length, that's 1664 blocks we have to individually flip over to Nonsecure State.
...
## Task 3 - Bootstrap the Nonsecure State

Nonsecure State and Secure State have their own copies of some important things:
...
the Secure Main Stack Pointer when you are Secure State, and is the nonsecure
Main Stack Pointer when you are in Nonsecure State.

But, Secure State also gets access to the Nonsecure State values as well, through
different names.
...
Nonsecure State have access to the Secure State peripherals?
...
The point of TrustZone is to keep things in Secure State (like encryption keys) a
secret from Nonsecure State (which might get hacked when processing untrusted
input).

Secure State is more trusted, and has more permissions that Nonsecure State.

</details>

To actually enter Nonsecure world, we need to:

* Set Nonsecure State's Vector Table Offset Register to where the code lives in
  flash, so that interrupts and exceptions work
* Set Nonsecure State's Main Stack Pointer, so it has a stack to use
* Zero all the CPU registers so we don't leak any secrets to Nonsecure State
...
from Secure State to Nonsecure State through this function? I hope not, but maybe
there's a bug!
...
That's because we haven't loaded a Nonsecure State binary! Let's do that.
...
still ran first. But it bounced us into Nonsecure State and our nonsecure
