...
✅ Open the [`stm32-code/standalone-app/src/bin/standalone-rtic-hello.rs`](../../../stm32-code/standalone-app/src/bin/standalone-rtic-hello.rs) file.
...
✅ Build the `standalone-rtic-hello` example and look at the generated `rtic-expansion.rs` file.

You can use `rustfmt` on `target/rtic-expansion.rs` to make the generated code easier to read.

```bash
cat target/rtic-expansion.rs | rustfmt | bat --language rust
# or, if you don't have "bat"
cat target/rtic-expansion.rs | rustfmt | less
```

Among other things, the file should contain the following lines. Note that interrupts are disabled during the execution of the `init` function:
