...
pub struct Board {
    ...
    pub leds: Leds,
    /// Timer
    pub timer: Timer<hal::peripherals::TIMER0>,
...
pub struct Timer<T: hal::timer::Instance>(hal::timer::Timer<'static, T>);

impl<T: hal::timer::Instance> DelayNs for Timer<T> {
...
impl<T: hal::timer::Instance> Timer<T> {
    /// Create a new timer instance which can be used for blocking delays.
    pub fn new(peri: Peri<'static, T>) -> Self {
...
pub fn init() -> Result<Board, Error> {
    ...
    config.lfclk_source = hal::config::LfclkSource::InternalRC;
